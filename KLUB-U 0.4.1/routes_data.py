...
class RouteManager:
    ...
    def get_next_signal(self, meters: int, distance_ahead: int = 2000) -> Optional[Dict]:
        """
        Получает ближайший светофор впереди с учётом направления движения
        Для четного направления (1): ищет сигналы с meters > текущих
        Для нечетного направления (2): ищет сигналы с meters < текущих
        Маршрут циклический - после конца переходит на начало
        """
        next_signals = []
        
        if self.direction == 1:  # Четные маршруты - едем в сторону увеличения метров
        ...
        else:  # Нечетные маршруты (direction == 2) - едем в сторону уменьшения метров
            # Ищем сигналы ПЕРЕД текущей позицией (с меньшими метрами)
            for signal in self.signals:
                if meters - distance_ahead <= signal['meters'] < meters:
                    next_signals.append(signal)
            
            if next_signals:
                # Возвращаем ближайший (с наибольшими метрами, т.е. первый впереди)
                closest_signal = max(next_signals, key=lambda s: s['meters'])
                return closest_signal
            
            # Если не нашли, маршрут циклический - ищем от конца
            max_meters = max([s['meters'] for s in self.signals]) if self.signals else 0
            for signal in self.signals:
                if signal['meters'] >= max_meters - distance_ahead:
                    next_signals.append(signal)
